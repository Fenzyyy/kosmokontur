...
function renderOrders(){let y=defaults.years,h="<table><thead><tr><th>Источник</th>"+y.map(x=>"<th>"+x+"</th>").join("")+"</tr></thead><tbody>";defaults.sources.forEach(s=>{h+="<tr><td><strong>"+s.name+"</strong><br><small>"+s.id+" · cap "+fmt(s.capacity)+" t/y</small></td>";y.forEach(yr=>{let v=plan.orders?.[s.id]?.[yr]??0;h+='<td><input type="number" min="0" step="0.1" data-s="'+s.id+'" data-y="'+yr+'" value="'+v+'"></td>'});h+="</tr>"});$("ordersEditor").innerHTML=h+"</tbody></table>";$("ordersEditor").querySelectorAll("input").forEach(i=>i.oninput=e=>{let s=e.target.dataset.s,y=Number(e.target.dataset.y);plan.orders[s]??={};plan.orders[s][y]=Number(e.target.value)||0;sync()})}
function renderReserve(){let y=defaults.years,h="<table><thead><tr><th>Источник</th>"+y.map(x=>"<th>"+x+"</th>").join("")+"</tr></thead><tbody>";defaults.sources.forEach(s=>{let disabled=s.requires_reservation?"":" disabled title=\"Для этого канала резерв всегда равен полной мощности\"";h+="<tr><td>"+s.name+"</td>";y.forEach(yr=>{let v=s.requires_reservation?(plan.reserved?.[s.id]?.[yr]??0):s.capacity;h+='<td><input type="number" min="0" step="0.1" data-s="'+s.id+'" data-y="'+yr+'" value="'+v+'"'+disabled+'></td>'});h+="</tr>"});$("reserveEditor").innerHTML=h+"</tbody></table>";$("reserveEditor").querySelectorAll("input:not([disabled])").forEach(i=>i.oninput=e=>{let s=e.target.dataset.s,y=Number(e.target.dataset.y);plan.reserved[s]??={};plan.reserved[s][y]=Number(e.target.value)||0;sync()})}
function investmentDefault(id){let o=defaults.investments.find(x=>x.id===id);if(!o?.default_schedule)throw new Error("Нет стандартного investment schedule для "+id);return clone(o.default_schedule)}
...
async function optimize(){msg("planMessage","Оптимизатор выполняется…");$("optimizeBtn").disabled=true;try{sync();let ids=["BASE"];if($("stressScenario").checked)ids.push("MANDATORY_STRESS");lastResult=await api("/api/optimize",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({plan:plan,scenario_ids:ids,run_frontier:true})});currentScenario="BASE";document.querySelectorAll(".scenario-btn").forEach(x=>x.classList.toggle("active",x.dataset.scenario==="BASE"));const stressButton=document.querySelector('.scenario-btn[data-scenario="MANDATORY_STRESS"]');if(stressButton)stressButton.disabled=!lastResult.scenarios?.MANDATORY_STRESS;plan=clone(lastResult.plan);editors();$("resultJson").textContent=JSON.stringify(lastResult,null,2);renderDashboard();renderFrontier();msg("planMessage",lastResult.feasible?"План найден без HARD-нарушений":"Найден лучший доступный план; проверьте dashboard",lastResult.feasible?"good":"");view("dashboard")}catch(e){msg("planMessage",e.message,"bad")}finally{$("optimizeBtn").disabled=false}}
function renderDashboard(){let r=lastResult?.scenarios?.[currentScenario];if(!r){$("dashboardContent").innerHTML='<div class="dashboard-empty"><h3>Нет результата для сценария</h3></div>';return}let k=r.kpis||{},good=r.feasible;$("dashboardContent").innerHTML='<div class="result-status '+(good?"good":"bad")+'"><div><strong>'+(good?"Допустимый план":"Есть HARD-нарушения")+'</strong><p>'+(r.meta?.name||currentScenario)+'</p></div><span class="status '+(good?"ok":"bad")+'">'+(good?"FEASIBLE":"CHECK")+'</span></div><div class="kpis"><div class="kpi"><div class="label">PV COST</div><div class="value">'+fmt(k.pv_cost)+' млн</div></div><div class="kpi"><div class="label">CAPEX</div><div class="value">'+fmt(k.capex_total)+' млн</div></div><div class="kpi"><div class="label">SHORTAGE</div><div class="value">'+fmt(k.shortage_total)+' т</div></div><div class="kpi"><div class="label">MIN SL TOTAL</div><div class="value">'+pct(k.min_sl_total)+'%</div></div><div class="kpi"><div class="label">MIN SL CRITICAL</div><div class="value">'+pct(k.min_sl_critical)+'%</div></div></div><div class="charts"><div class="card"><div class="card-head"><div><h3>Запас топлива</h3><span>Запас и требуемый резерв.</span></div></div><div class="chart-box"><div id="inventoryChart" class="svg-chart"></div></div></div><div class="card"><div class="card-head"><div><h3>CAPEX</h3><span>Капитальные затраты по годам.</span></div></div><div class="chart-box"><div id="capexChart" class="svg-chart"></div></div></div></div><div class="two-col"><div class="card"><div class="card-head"><h3>Годовая физика</h3></div><div id="yearlyTable" class="table-scroll"></div></div><div class="card"><div class="card-head"><h3>Нарушения / проверки</h3></div><div id="violations"></div></div></div>';drawCharts(r);renderYearly(r);renderViolations(r)}
...
document.querySelectorAll(".scenario-btn").forEach(b=>b.onclick=()=>{if(b.disabled)return;currentScenario=b.dataset.scenario;document.querySelectorAll(".scenario-btn").forEach(x=>x.classList.toggle("active",x===b));renderDashboard()});
